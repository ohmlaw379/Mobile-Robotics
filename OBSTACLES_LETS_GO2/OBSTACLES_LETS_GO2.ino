...
int turn[5][4] = {
  {255, 167, 255, 177}, //forward //  {255, 152, 255, 164}, //forward 
...
void avoid_obstacles(int current_junction, int next_junction);
...
if (stage < 2){
    ...
    while (count < return_to_junction){
      follow_the_line();
    }
    if (count == return_to_junction){
      stop();
      delay(10000);
      stage = 1;
    }
...
//if it is coming from 1 or not
  ...
  if (distance_sensor_reading > 2300){
       turn180_function();
       Serial.println(distance_sensor_reading);
       Serial.println("....................");
       old_direction = -direction;
       old_mid_direction = -mid_direction; 
       int turn_direction = -direction_of_turn;
       return_to_junction = no_junctions_pass - count;
       quota = junctions_until_turn;
       delay(5);
       avoid_obstacles(current_junction, route[r]);
    }
